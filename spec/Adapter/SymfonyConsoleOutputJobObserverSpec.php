...
class SymfonyConsoleOutputJobObserverSpec extends ObjectBehavior {
    ...
    public function it_should_output_starting_event(OutputInterface $output, JobHandle $jobHandle): void {
        $jobHandle->id()->willReturn(32);

        $output->writeln('Starting job 32')->shouldBeCalled();

        $this->starting($jobHandle);
    }

    public function it_should_output_finished_event(OutputInterface $output, JobHandle $jobHandle): void {
        $jobHandle->id()->willReturn(32);
