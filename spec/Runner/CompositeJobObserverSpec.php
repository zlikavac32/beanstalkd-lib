...
class CompositeJobObserverSpec extends ObjectBehavior
{
    ...
    public function it_should_propagate_starting_event(
        JobObserver $firstObserver,
        JobObserver $secondObserver,
        JobHandle $jobHandle
    ): void {
        $firstObserver->starting($jobHandle)
            ->shouldBeCalled();
        $secondObserver->starting($jobHandle)
            ->shouldBeCalled();

        $this->starting($jobHandle);
    }

    public function it_should_propagate_finished_event(
        JobObserver $firstObserver,
        JobObserver $secondObserver,
        JobHandle $jobHandle
    ): void {
        $firstObserver->finished($jobHandle, 4.9)
            ->shouldBeCalled();
        $secondObserver->finished($jobHandle, 4.9)
            ->shouldBeCalled();

        $this->finished($jobHandle, 4.9);
    }

    public function it_should_propagate_failed_event(
        JobObserver $firstObserver,
        JobObserver $secondObserver,
        JobHandle $jobHandle
    ): void {
        ...
        $firstObserver->failed($jobHandle, $e, 5.2)
            ->shouldBeCalled();
        $secondObserver->failed($jobHandle, $e, 5.2)
            ->shouldBeCalled();
    ...
    {
        $this->has($firstObserver)
            ->shouldReturn(true);
        $this->has($secondObserver)
            ->shouldReturn(true);
    }
